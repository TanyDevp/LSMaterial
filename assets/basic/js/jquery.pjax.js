...
(function (window, undefined) {
    ...
    var core = {
        ...
        turn: function (url, data, callback) {
            ...
            //页面跳转
            //--------------------------------------------------------------------------------------------------------
            var turn = url.substring(url.indexOf(location.host) + location.host.length).replace('#', 'pages');
            if (url.indexOf('#/') > 0) {
                url = url.indexOf('?') > -1 ? turn.replace('?', '.html?') : turn + '.html';
            }
            else if (turn == '/' || turn == '/pages') {
                url = '/pages/main.html';
            }
            //--------------------------------------------------------------------------------------------------------
            ...
            if (core.fnb && config.cache) {
                ...
                if (value !== null) {
                    core.show(url, value.title, value.html);
                ...
                if (xhr.readyState === 4) {
                    ...
                    if (xhr.status >= 200 && xhr.status < 300 || xhr.status === 304) {
                        ...
                        if (config.filter.content && !config.filter.content(title, html)) {
                            ...
                            // 显示新页面
                            core.show(url, title, html);

                            if (!core.fnb) {
                                // 修改URL
                                if (suppost === SUPPORT.HTML5) {
                                    var rurl = url.replace('pages/', '#/').replace('.html', '');
                                    if (url.indexOf('main') > -1) {
                                        rurl = '/';
                                    }
                                    history.pushState(null, null, rurl);
                                } else {
